    ...
    <div className="absolute inset-0 pointer-events-none">
      <div className="absolute top-24 left-1/2 w-[920px] max-w-[94vw] -translate-x-1/2 pointer-events-auto">
        <div className="glass-panel space-y-3 px-5 py-4">
          <PrincipleFlow active="evolution" compact />
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <div className="mt-2 text-2xl font-black text-[#26323f]">
                {physicsSnapshot ? (
                  <>
                    <HexagramName name={physicsSnapshot.hexagram}>{physicsSnapshot.hexagram}</HexagramName>
                    {' / '}
                    {formatBits(physicsSnapshot.bits, physicsSnapshot.display_bits)}
                  </>
                ) : (
                  '等待一次物理模拟'
                )}
      ...
      <div className="absolute top-[240px] bottom-6 left-1/2 w-[920px] max-w-[94vw] -translate-x-1/2 pointer-events-auto overflow-y-auto pr-1">
