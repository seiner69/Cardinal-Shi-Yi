...
function QueryInput() {
  ...
  return (
    <form onSubmit={handleSubmit} className="glass-panel p-2">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center lg:flex-col lg:items-stretch 2xl:flex-row 2xl:items-center">
...
function ResultsPanel() {
  ...
  const targetHexName = fsmData
    ? (fsmData.target_hexagram?.trim() || deterministic?.current_node?.name?.trim() || null)
    : null
  ...
  if (!fsmData) {
    return (
      <>
        <div className="pointer-events-auto flex flex-col gap-3 lg:max-h-[calc(100vh-128px)] lg:w-[420px] lg:overflow-y-auto">
          <QueryInput />
          <PrincipleFlow active="analysis" />
          <section className="glass-panel px-5 py-4">
            <div className="panel-title">语义分析层</div>
            <div className="mt-2 text-2xl font-black text-[#26323f]">等待材料</div>
            <div className="mt-2 text-sm leading-relaxed text-[#6b6259]">
              输入一个历史事件、现实系统或博弈困局，系统会抽取内外系统与 B1-B6 状态，并生成可带入模拟页的原始物理输入。
            </div>
          </section>
        </div>
        ...
        <div className="pointer-events-auto flex flex-col gap-3 lg:max-h-[calc(100vh-128px)] lg:w-[420px] lg:overflow-y-auto">
          <section className="glass-panel p-4">
            <div className="panel-title mb-3">输出等待区</div>
            <div className="rounded-md border border-[#524639]/10 bg-white/50 px-3 py-3 text-sm leading-relaxed text-[#6b6259]">
              分析完成后，这里会显示六层赋值依据、推演策略、翻转预览和 RAG 检索结果。
            </div>
          </section>
        </div>
      </>
  ...
  return (
    <>
      <div className="pointer-events-auto flex flex-col gap-3 lg:max-h-[calc(100vh-128px)] lg:w-[420px] lg:overflow-y-auto">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <div className="mt-2 text-2xl font-black text-[#26323f]">
                {targetHexName ? (
                  <HexagramName name={targetHexName}>{targetHexName}</HexagramName>
                ) : (
                  '未知态'
                )}
              </div>
        ...
        {deterministic && (
          <section className="glass-panel grid grid-cols-2 gap-2 p-4 text-center">
          ...
          <section className="glass-panel p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {simulateFlips.map((flip) => (
                <div
                  key={flip.bit}
                  className="min-w-[84px] rounded-md border border-[#524639]/15 bg-white/50 px-3 py-2 text-center transition hover:border-[#9f1239]/50"
                >
                  <span className="block font-mono text-[8px] text-[#8a8177]">B{flip.bit}</span>
                  <HexagramName name={flip.hexagram} className="block w-full text-center text-base font-bold text-[#26323f]">
                    {flip.hexagram}
                  </HexagramName>
                  <span className="block font-mono text-[8px] text-[#8a8177]">{formatBits(flip.new_bits, flip.display_new_bits)}</span>
                  <button
                    type="button"
                    onClick={() => handleFlipSelect(flip)}
                    className="mt-2 rounded border border-[#9f1239]/25 px-2 py-1 font-mono text-[8px] text-[#9f1239] transition hover:bg-[#9f1239]/10"
                  >
                    带入
                  </button>
                </div>
              ...
              {retrievalResults.slice(0, 3).map((item, index) => (
                <div key={`${item.hexagram_name}-${index}`} className="text-xs leading-relaxed text-[#4f5d6a]">
                  <span className="font-mono text-[10px] text-[#8a8177]">
                    [
                    <HexagramName name={item.hexagram_name} className="font-mono text-[10px] text-[#8a8177]">
                      {item.hexagram_name ?? '未知'}
                    </HexagramName>
                    {' '}
                    / {item.text_type ?? '文本'}]
                  </span>
...
export function AnalysisView() {
  return (
    <div className="absolute left-4 right-4 top-24 bottom-6 pointer-events-none overflow-y-auto pr-1 lg:left-6 lg:right-6 lg:overflow-visible">
